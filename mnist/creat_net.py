...
base_root='/home/dana/caffe_wds/mnist'
train_data=os.path.join(base_root,'data/mnist_train_lmdb')
test_data=os.path.join(base_root,'data/mnist_test_lmdb')
...
def creat_train_net(train_data, batch_size):
    n=caffe.NetSpec()
    n.data,n.label=L.Data(source=train_data,batch_size=batch_size,ntop=2,
                   backend=P.Data.LMDB,transform_param=dict(scale=1./255))
...
def creat_test_net(test_data, batch_size):
    n=caffe.NetSpec()
    n.data,n.label=L.Data(source=test_data,batch_size=batch_size,ntop=2,
                   backend=P.Data.LMDB,transform_param=dict(scale=1./255))
    n.conv1=L.Convolution(n.data,kernel_size=5,stride=1,num_output=20,
             weight_filler=dict(type='xavier'))
    n.pool1=L.Pooling(n.conv1,kernel_size=2,stride=2,pool=P.Pooling.MAX)
